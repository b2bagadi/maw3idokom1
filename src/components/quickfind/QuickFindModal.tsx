...
export function QuickFindModal({ onClose }: QuickFindModalProps) {
  ...
  useEffect(() => {
    ...
    socket.off('quickfind_results');
    socket.off('quickfind_error');
    socket.off('request_created');
    socket.off('request_error');
    socket.off('request_offered');
    ...
    socket.on('request_error', () => {
      alert(`❌ ${t('quickFind.errorSend')}`);
      setLoading(false);
    });

    socket.on('request_offered', ({ businessId, businessName, price, address, lat, lng, logoUrl, requestId }) => {
      ...
      // Store requestId for confirmation
    ...
    socket.on('booking_confirmed', ({ bookingId }) => {
        ...
        setRequestSent(false);
          ...
          const targetBusiness = lastSelectedBusinessRef.current || selectedBusiness;
          if (targetBusiness?.lat && targetBusiness?.lng) {
            const mapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${targetBusiness.lat},${targetBusiness.lng}`;
            window.open(mapsUrl, '_blank');
            // Don't return, allow redirect to dashboard so user can see their new booking
          }

          router.push('/dashboard');
    });

    socket.on('booking_error', ({ message }) => {
        alert('❌ ' + t('quickFind.bookingFailed', { message }));
        setLoading(false);
    });
  ...
  const handleConfirmBooking = (business: Business) => {
    ...
    setLoading(true); // Show loading state while confirming
