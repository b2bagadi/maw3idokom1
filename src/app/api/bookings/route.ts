...
export async function PUT(req: Request) {
    ...
    try {
        ...
        if ((session.user.role === 'BUSINESS' || session.user.role === 'ADMIN') && ['CONFIRMED', 'REJECTED', 'COMPLETED'].includes(status)) {
            ...
            if (status === 'CONFIRMED') {
                notificationText = `Your booking for ${booking.service.name} has been confirmed`;
            } else if (status === 'REJECTED') {
                notificationText = `Your booking for ${booking.service.name} has been rejected`;
            } else if (status === 'COMPLETED') {
                notificationText = `Your booking for ${booking.service.name} is complete! Please leave a review`;
            }
