...
export async function POST(req: Request) {
    try {
        ...
        const subSetting = await prisma.globalSettings.findUnique({
            where: { key: 'SUBSCRIPTION_PLANS_ENABLED' }
        });
        const subscriptionEnabled = subSetting?.valueEn === 'true';

        return NextResponse.json(
            { 
                message: 'Business registered successfully', 
                userId: result_1.user.id,
                subscriptionEnabled
            },
            { status: 201 }
        );
